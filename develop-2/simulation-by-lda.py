...
def scale_angles(records, angle_ranges=angle_ranges):
    # ! We do not change the input
    records = records.copy()

    for k in angle_ranges:
        min, max, _, _ = angle_ranges[k]
        records[k] = (records[k] - min) / (max - min)

    return records


def shrink_records(raw_records, count=2000):
    records = scale_angles(raw_records)
    ...
    new_records = raw_records.iloc[order[:count]]
...
records = scale_angles(new_records)
